...
def setup_argparse():
    parser = argparse.ArgumentParser(description="Train EfficientNet on VisuoTactile Dataset.")
    ...
    parser.add_argument('--model_name', type=str, default='efficientnet_b0', 
                        ...
                        help='EfficientNet model architecture to use.')
...
def main():
    ...
    run_dir = os.path.join(args.output_dir, f"{args.model_name}_run_{timestamp}")
    ...
    # EfficientNet b0 uses 224x224, higher variants might benefit from larger resolution 
    # but 224 is safe for all and matches original script behavior.
    data_transforms = {
    ...
    model_fn = getattr(models, args.model_name)
    try:
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val':
                if epoch_loss < best_loss:
                    ...
                    best_acc = epoch_acc
                    best_model_wts = copy.deepcopy(model.state_dict())
                    best_model_path = os.path.join(run_dir, f"best_{args.model_name}_model.pth")
