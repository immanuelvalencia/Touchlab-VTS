"""Fit and optimize Naive Bayes from hard-label touch trials."""
...
from algorithms.naive_bayes.config import NaiveBayesConfig, load_config
from algorithms.naive_bayes.factory import DEFAULT_CONFIG_PATH
...
def _mapping(config, *, name: str, likelihoods, class_prior, temperature: float) -> dict:
    return {
        "schema_version": 1,
        "name": name,
...
def optimize(train, validation, base, *, smoothing_values, permutations, seed, name):
    ...
    for smoothing in smoothing_values:
        ...
        def objective(log_temperature):
            ...
            config = NaiveBayesConfig.from_mapping(mapping)
            metrics = evaluate_prefixes(
                runs,
                shapes=config.shapes,
                create_classifier=lambda: NaiveBayesClassifier(config),
            )
    ...
    report = {
        "method": "naive_bayes_feature_likelihoods",
...
def main(argv=None) -> int:
    ...
    config_path = write_json(run_dir / f"naive_bayes_config_{run_timestamp}.json", optimized)
    ...
    if args.test:
        ...
        config = NaiveBayesConfig.from_mapping(optimized, source_path=config_path)
    ...
    sidecar = install_model_sidecar(config_path, args.model, ".nb.json") if args.model else None
