...
from algorithms.bag_of_features.factory import create_algorithm
from algorithms.registry import get_algorithm


class BagOfFeaturesTests(unittest.TestCase):
    def test_probabilities_are_normalized_and_aliases_are_supported(self):
        algorithm = create_algorithm()

        result = algorithm.update("multiface_vertex")

        self.assertAlmostEqual(sum(result.values()), 1.0, places=12)
        self.assertEqual(algorithm.touch_count, 1)

    def test_fixed_multiset_is_permutation_invariant(self):
    ...
    def test_reset_removes_previous_histogram(self):
        algorithm = create_algorithm()
    ...
    def test_registry_prefers_model_sidecar(self):
        spec = get_algorithm("bag_of_features")
        default_path = Path(__file__).parents[1] / "algorithms" / "bag_of_features" / "config.json"
        with tempfile.TemporaryDirectory() as directory:
            model_path = Path(directory) / "features.pth"
            model_path.write_bytes(b"")
            sidecar_path = model_path.with_suffix(".bof.json")
